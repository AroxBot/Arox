...
export default Handler.SlashCommandHandler({
  ...
  execute: async (options, interaction) => {
    ...
    const buttons = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("guessnumber_1").setEmoji(options._e("gn1")).setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("guessnumber_2").setEmoji(options._e("gn2")).setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("guessnumber_3").setEmoji(options._e("gn3")).setStyle(ButtonStyle.Secondary)
    );
    ...
    collector.on("collect", async (i) => {
      ...
      setTimeout(async () => {
        resultMessage = `${options._t("commands.guessnumber.messages.bet", {
          ...
          won
            ? options._t("commands.guessnumber.messages.win", {
    ...
    collector.on("end", async (collected) => {
      ...
      disabledButtons.addComponents(
        new ButtonBuilder().setCustomId("guessnumber_1").setEmoji(options._e("gn1")).setStyle(ButtonStyle.Secondary).setDisabled(true),
        new ButtonBuilder().setCustomId("guessnumber_2").setEmoji(options._e("gn2")).setStyle(ButtonStyle.Secondary).setDisabled(true),
        new ButtonBuilder().setCustomId("guessnumber_3").setEmoji(options._e("gn3")).setStyle(ButtonStyle.Secondary).setDisabled(true)
      );
    });
  }
});
