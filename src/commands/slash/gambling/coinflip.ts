...
export default Handler.SlashCommandHandler({
    ...
    execute: async (options, interaction) => {
        ...
        if (!user || user.cash < amount) {
            return await throwError(interaction, options, options._t("commands.coinflip.messages.not_enough_money"));
        }
        ...
        collector.on("collect", async (i) => {
            ...
            setTimeout(async () => {
                ...
                })} ${won ? options._t("commands.coinflip.messages.win", {
        ...
        collector.on("end", async (collected) => {
            if (collected.size === 0) {
                const timeoutContainer = new ContainerBuilder().addTextDisplayComponents(d => d.setContent(options._t("commands.coinflip.messages.timeout")));
