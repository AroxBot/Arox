...
export default Handler.SlashCommandHandler({
  ...
  execute: async (options, interaction) => {
    ...
    const confirmationEmbed = new EmbedBuilder()
      ...
      .setDescription(
        options._t("commands.transfer.messages.confirmation.description", {
          ...
          "\n\n" +
          "⚠️ | **" +
          options._t("commands.transfer.messages.confirmation.warning_title") +
          "**: *" +
          options._t("commands.transfer.messages.confirmation.warning_body") +
          "*"
      )
      .setColor(options._c("default"))
      .setFooter({
        text: options._t("commands.transfer.messages.confirmation.footer", { user: sender.displayName }),
      ...
      if (i.customId === "approve") {
        ...
        if (!senderAfterCollect || senderAfterCollect.cash < amount) {
          return await interaction.editReply({
            content: options._t("commands.transfer.messages.error.not_enough_money_after_confirm"),
            embeds: [],
            components: []
          });
        }
        ...
        await interaction.editReply({
          content: options._t("commands.transfer.messages.success", {
            sender: sender.toString(),
            recipient: recipient.toString(),
            amount: amount.toLocaleString()
          }),
