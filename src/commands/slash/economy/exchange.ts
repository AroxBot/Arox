...
function createExchangeRow(options: any, coins: any[]): ActionRowBuilder<StringSelectMenuBuilder> {
    ...
    for (const coin of coins) {
        ...
        const option = new StringSelectMenuOptionBuilder()
            .setLabel(capitalizeWordsRegex(coin.name))
            .setValue(coin.id);
...
async function handleCoinSelection(selectInteraction: StringSelectMenuInteraction, options: any) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: `💵 ${options._t("commands.exchange.messages.current_price")}`, value: `${options._e("cash")} ${coin.cost.toLocaleString()}`, inline: true },
            { name: `📍 ${options._t("commands.exchange.messages.previous_price")}`, value: `${options._e("cash")} ${previousPrice.toLocaleString()}`, inline: true },
            { name: `📤 ${options._t("commands.exchange.messages.launch_cost")}`, value: `${options._e("cash")} ${coin.launchCost.toLocaleString()}`, inline: true },
            { name: `📈 ${options._t("commands.exchange.messages.highest_value")}`, value: `${options._e("cash")} ${coin.highestValue.toLocaleString()}`, inline: true },
            { name: `📉 ${options._t("commands.exchange.messages.lowest_value")}`, value: `${options._e("cash")} ${coin.lowestValue.toLocaleString()}`, inline: true }
        )
        .setAuthor({
    ...
    const actionRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
            .setEmoji(options._e("buttonUndo"))
            .setStyle(ButtonStyle.Secondary)
            .setCustomId(`exgeri_${selectInteraction.message.id}`),
        new ButtonBuilder()
                ...
                new ActionRowBuilder<TextInputBuilder>()
                    .addComponents(
                        new TextInputBuilder()
                            ...
                            .setLabel(
                                action === 'al'
                                    ? options._t("commands.exchange.messages.buy_amount_label")
                                    : options._t("commands.exchange.messages.sell_amount_label")
                            )
