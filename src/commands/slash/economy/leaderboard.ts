...
export default Handler.SlashCommandHandler({
    ...
    execute: async (options, interaction) => {
        ...
        const generateEmbed = async (page: number) => {
            ...
            const description = await Promise.all(leaderboardSlice.map(async (user, index) => {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: options._t("commands.leaderboard.messages.user_info"), value: `${options._t("commands.leaderboard.messages.user_balance")} ${userCash.toLocaleString()}\n${options._t("commands.leaderboard.messages.user_rank")} #${userRank || "N/A"}` },
                    { name: " ", value: " " },
                    { name: options._t("commands.leaderboard.messages.global_leaderboard"), value: description.join('\n') || options._t("commands.leaderboard.messages.no_one_in_leaderboard") }
                )
                .setFooter({ text: options._t("commands.leaderboard.messages.footer", { user: interaction.user.username, page: page.toString(), totalPages: totalPages.toString() }) });
        ...
        const message = await interaction.reply({ embeds: [initialEmbed], components: [new ActionRowBuilder<ButtonBuilder>().addComponents(initialButtons.components)], fetchReply: true });
